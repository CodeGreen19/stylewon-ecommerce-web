...
const sendOTPschema = z.object({
  phoneNo: z
    .string()
    .refine((v) => bdPhoneRegex.test(v), "Invalid phone number"),
});

export default function AddOrUpdatePhoneNo({
  phoneNumber,
}: {
  phoneNumber: string;
}) {
  return (
    <div>
      {phoneNumber ? <PhoneUpdate phoneNumber={phoneNumber} /> : <PhoneAdd />}
    </div>
  );
}

function PhoneUpdate({ phoneNumber }: { phoneNumber: string }) {
  const [type, setType] = useState<CompType>("SEND");
  const [newPhoneNo, setNewPhoneNo] = useState(phoneNumber);
  return (
    <div>
      {type === "SEND" ? (
        <SendOTPToChange
